...
const bySchema = (type, registry) => (topic, schema, msg, parseOptions = null) => (() => {
  ...
  console.log(`Run Id: ${runId}, Parsed schema: ${JSON.stringify(parsedSchema)}, Schema: ${schemaString}`);
  let id = registry.cache.getBySchema(parsedSchema);
  if (id) {
    console.log(`Run Id: ${runId}, Id cached: ${id}`);
    return Promise.resolve(id)
  }
  
  if (type === "multi"){
    console.log(`Run Id: ${runId}, multi`);
    if (schema.namespace){
        type = `${schema.namespace}.${schema.name}`;
        console.log(`Run Id: ${runId}, namespace, type: ${type}`);
    } else{
        type = `${schema.name}`;
        console.log(`Run Id: ${runId}, no namespace, type: ${type}`);
    }
  }

  return pushSchema(registry, topic, schemaString, type, runId)
